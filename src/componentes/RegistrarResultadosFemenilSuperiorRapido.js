...
const RegistrarResultadosFemenilSuperiorRapido = () => {
        ...
        const fetchPartidos = async () => {
          try {
            ...
            const querySnapshot = await getDocs(
              query(partidosRef, where('jornada', '==', jornada), where('categoria', '==', 'Femenil'), where('nivelAcademico', '==', 'Superior'),where('modalidadTorneo', '==', 'Fútbol Rápido'))
            );
    ...
    const handleVerPartidos = async () => {
      ...
      const handleResultadoChange = (partidoId, campo, valor) => { //Input para goles local y goles visitante
      ...
      const handleGanadorPenalesChange = (partidoId, campo, valor) => { //Checkbox para ganador en shoot out 
        setResultados((prevState) => {
          const nuevosResultados = { ...prevState };
          if (!nuevosResultados[partidoId]) {
            nuevosResultados[partidoId] = {};
    ...
    return ( 
        <div className="hero">
            ...
            <img src="https://tinyurl.com/2obtocwe" alt=''/>
              <center><h2> Registro de Resultados Fútbol Rápido Femenil Nivel Superior</h2>
